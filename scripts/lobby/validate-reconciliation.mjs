...
const baseline='0f44cb134e7650cffe6dc961e272c16679e71141';
...
assert.match(fs.readFileSync('config/lobby-release.ts','utf8'),/lobbyReleased = false/);
...
const d=JSON.parse(fs.readFileSync('docs/brand/lobby-reconciliation-browser.json'));
assert.equal(d.status,'reconciliation passed');assert.equal(d.reconciliation.length,26);assert.deepEqual(d.errors,[]);
for(const row of d.reconciliation){let ready=-1,start=-1;for(const event of row.events??[]){
 if(event.event==='PREPARATION_START')start=event.ms;
 ...
 if(event.event==='CLOCK_FIRST')assert.ok(event.ticks===0&&event.accumulator===0);
