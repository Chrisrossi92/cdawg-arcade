...
const out='tmp/release-b-reconciliation',rows=[];const b=await browser(true,{diagnosticsPath:out+'/compiled-cdp.jsonl'});
try{
 await b.send('Network.enable');await b.send('Network.clearBrowserCache');
 for(const cache of ['cold','warm']){
  await b.send('Page.navigate',{url:'http://127.0.0.1:5247/'});await b.until("!!document.querySelector('.arcade-lobby')");await sleep(300);
 ...
 await b.send('Page.navigate',{url:'http://127.0.0.1:5247/review.html'});await b.until("!!document.querySelector('#report')?.textContent");await b.click('Run viewport matrix');await b.until("!!document.querySelector('#matrix-report')",120000);
 const viewport=JSON.parse(await b.evaluate("document.querySelector('#matrix-report').textContent"));assert.equal(viewport.status,'passed');assert.equal(viewport.matrix.length,4);
 fs.writeFileSync(out+'/compiled-browser.json',JSON.stringify({status:'passed',rows,viewport},null,2));console.log(JSON.stringify({status:'passed',runs:4,viewportCases:4,entryMs:rows.map(r=>({cache:r.cache,ms:r.entryMs}))}));
