...
const run=(name,args,{command=process.execPath,preserve=[]}={})=>{
 ...
 try{const output=execFileSync(command,args,{env,encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:8*1024*1024});writeFileSync(`tmp/audio-readiness/${name}.log`,output);outcomes.push({name,pass:true});console.log('PASS',name);}
 catch(error){writeFileSync(`tmp/audio-readiness/${name}.log`,String(error.stdout??'')+String(error.stderr??''));outcomes.push({name,pass:false});console.log('FAIL',name);}
 finally{for(const [path,bytes]of prior){writeFileSync(`tmp/audio-readiness/${path.split('/').at(-1)}`,readFileSync(path));writeFileSync(path,bytes);}writeFileSync('tmp/audio-readiness/suites.json',JSON.stringify(outcomes,null,2)+'\n');}
};
...
run('graph',['scripts/lobby/validate-release-b.mjs']);
