...
stem='cdawg-mascot-correction-v004'
...
for p in sorted(list((left/'frames').glob('*.png'))+list((left/'views').glob('*.png'))+list((left/'expressions').glob('*.png'))):
 ...
 if rel.name in {'gameplay.png','lobby-host.png'}:
  ys,xs=np.where(x[:,:,3]>.02);margin=int(min(xs.min(),ys.min(),x.shape[1]-1-xs.max(),x.shape[0]-1-ys.max()));minimum=min(minimum,margin);assert margin>=2,f'Clipped frame: {rel}'
comparisons=[]
for name,orig in [('default','expr-default.png'),('panic','expr-panic.png')]:
 x=pixels(left/'views'/orig);y=pixels(Path(a.reimport)/f'expr-{name}.png');err=float(np.abs(x-y).mean());assert err<.02,f'GLB import render mismatch: {name}: {err}';comparisons.append({'pose':name,'meanAbsoluteRGBAError':err,'threshold':.02})
assert len(files)==10,'Incomplete render inventory'
