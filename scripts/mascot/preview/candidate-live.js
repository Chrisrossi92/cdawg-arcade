...
const camera=new THREE.OrthographicCamera(-.56,.56,.56,-.56,.01,10);camera.position.set(.65,.85,1.85);camera.lookAt(0,.38,0);
...
const gltf=await new GLTFLoader().loadAsync('/cdawg-mascot-candidate-v002.glb');scene.add(gltf.scene);
...
const picker=document.querySelector('#live-pose');for(const name of Object.keys(actions)){const option=document.createElement('option');option.value=name;option.textContent=name.replaceAll('_',' ');picker.append(option)}picker.value='idle_default';picker.addEventListener('change',()=>state.pose=picker.value);
for(const a of Object.values(actions)){a.setLoop(THREE.LoopOnce,1);a.clampWhenFinished=true;a.play();a.paused=true;a.enabled=true;a.setEffectiveWeight(0)}
...
function draw(t){requestAnimationFrame(draw);if(document.hidden)return;const sample=measure(t);for(const a of Object.values(actions))a.setEffectiveWeight(0);
 ...
 else{const a=actions[state.pose];a.time=Math.min(clock(t),a.getClip().duration);a.setEffectiveWeight(1)}
