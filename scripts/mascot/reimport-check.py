...
p=argparse.ArgumentParser();p.add_argument('--candidate',action='store_true');p.add_argument('--input',required=True);p.add_argument('--output',required=True);a=p.parse_args(sys.argv[sys.argv.index('--')+1:]);source=Path(a.input);out=Path(a.output);out.mkdir(parents=True,exist_ok=True)
stem='cdawg-mascot-candidate-v002' if a.candidate else 'cdawg-mascot-feasibility-v001'
bpy.ops.wm.open_mainfile(filepath=str(source/(stem+'.blend')))
for o in list(bpy.data.objects):
 if o.type in {'MESH','ARMATURE'}:bpy.data.objects.remove(o,do_unlink=True)
for action in list(bpy.data.actions):bpy.data.actions.remove(action)
bpy.ops.import_scene.gltf(filepath=str(source/(stem+'.glb')))
rig=next(o for o in bpy.data.objects if o.type=='ARMATURE');actions=list(bpy.data.actions)
assert len(actions)==(26 if a.candidate else 5)
...
for name in ['idle_default','panic']:
 ...
 bpy.context.scene.frame_set(0 if name=='idle_default' else (23 if a.candidate else 24));bpy.context.scene.render.filepath=str(out/f'reimport-{name}.png');bpy.ops.render.render(write_still=True)
