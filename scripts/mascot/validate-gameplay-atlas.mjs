...
const root='assets/brand/mascot/runtime';let total=0;
for(const [group,count] of [['balance0',58],['balance1',58],['reactions0',87],['reactions1',62]]){
 const m=JSON.parse(readFileSync(`${root}/cdawg-mascot-${group}-v003.json`));const {data,info}=await sharp(`${root}/cdawg-mascot-${group}-v003.webp`).raw().toBuffer({resolveWithObject:true});assert.equal(info.width,m.meta.size.w);assert.equal(info.height,m.meta.size.h);assert.ok(info.width<=2048&&info.height<=2048);assert.equal(Object.keys(m.frames).length,count);
...
for(let p=0;p<4;p++)for(let i=0;i<29;i++){const group=`balance${Math.floor(p/2)}`;const m=JSON.parse(readFileSync(`${root}/cdawg-mascot-${group}-v003.json`));assert.ok(m.frames[`balance-${p}-${String(i).padStart(2,'0')}`])}
