...
execFileSync('mkdir',['-p','tmp/iphone']);
const outcomes=[];
const run=(name,args,{command=process.execPath,preserve=[]}={})=>{
 const prior=preserve.map(path=>[path,readFileSync(path)]);
 try{const output=execFileSync(command,args,{env,encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:8*1024*1024});writeFileSync(`tmp/iphone/${name}.log`,output);outcomes.push({name,pass:true});console.log('PASS',name);}
 catch(error){writeFileSync(`tmp/iphone/${name}.log`,String(error.stdout??'')+String(error.stderr??''));outcomes.push({name,pass:false});console.log('FAIL',name);}
 finally{for(const [path,bytes]of prior){writeFileSync(`tmp/iphone/${path.split('/').at(-1)}`,readFileSync(path));writeFileSync(path,bytes);}writeFileSync('tmp/iphone/suites.json',JSON.stringify(outcomes,null,2)+'\n');}
};
...
run('glb',['scripts/mascot/validate-correction-glb.mjs','assets/brand/mascot/correction-v004/source/cdawg-mascot-correction-v004.glb','tmp/iphone/glb.json']);
run('graph',['scripts/iphone/validate.mjs']);
