...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif path.startswith('/lobby/'): file=Path('/private/tmp/cdawg-audio-release-b/tmp/lobby-integration/harness')/path.removeprefix('/lobby/')
  else: file=root/'tmp/readiness/harness'/path.lstrip('/')
  if not file.is_file() or not (file in [root/'scripts/readiness/observer.js',root/'scripts/readiness/matrix.html'] or any(file.resolve().is_relative_to(base) for base in [root/'tmp/readiness/harness',Path('/private/tmp/cdawg-audio-release-b/tmp/lobby-integration/harness'),Path('/private/tmp/cdawg-audio-baseline/tmp/readiness/harness')])): self.send_error(404);return
