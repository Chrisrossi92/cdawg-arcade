...
import {stopBrowserProcess} from './cdp-process.mjs';
import {devToolsPort} from './cdp-port.mjs';
...
export async function browser(headless = true, {diagnosticsPath} = {}) {
  ...
  try {
    let port;
    for (let i=0; port===undefined; i++) {
      if (i>100 || child.exitCode !== null) throw Error('Disposable Chromium unavailable');
      if(existsSync(join(profile,'DevToolsActivePort')))port=devToolsPort(readFileSync(join(profile,'DevToolsActivePort'),'utf8'));
    ...
    connection=transport(socket,{record});const send=connection.send;
    ...
    return {send,evaluate,until,click,async close(){try{if(socket.readyState===WebSocket.OPEN)await send('Browser.close');}catch{}socket.close();connection.dispose();record({event:'session-cleanup',pending:connection.pendingCount()});await stopBrowserProcess(child);rmSync(profile,{recursive:true,force:true,maxRetries:10,retryDelay:200});}};
  } catch(error) {socket?.close();connection?.dispose();await stopBrowserProcess(child);rmSync(profile,{recursive:true,force:true,maxRetries:10,retryDelay:200});throw error;}
}
