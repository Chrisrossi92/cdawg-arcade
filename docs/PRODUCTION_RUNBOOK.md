...
Current accepted application: mascot integration `615898dcc24214206774e551b5bfb935171eefa1`. The single-guild official-score configuration is preserved. Automated release gates and the owner Discord canary passed; see [mascot release acceptance](brand/MASCOT_RELEASE_ACCEPTANCE.md) for precise data evidence and limitations. Acceptance documentation is merged without redeployment. The phase-specific status statements below are historical.

Latest accepted application: Phase 4D `73390d0`, with sessions enabled and attempts, official scoring and the temporary PKCE probe disabled. See [Phase 4D acceptance](PHASE_4D_ATTEMPT_REPLAY.md) and [attempt/replay architecture](ATTEMPT_REPLAY_ARCHITECTURE.md). Follow the [owner operating policy](../AGENTS.md); routine validation and authorized reversible deployments proceed autonomously. The earlier phase history follows.

Phase 3C established the paid Render service `cdawg-arcade-production` at `https://arcade.cdawgbot.xyz`, serving frontend and API from one origin. DNS/TLS and owner-observed Discord desktop acceptance passed. See [Phase 3C deployment evidence](PHASE_3C_PRODUCTION_DEPLOYMENT.md) for the exact accepted revision, rollback result, and operational limitations. Automatic deployment is off; the existing VPS remains untouched. Gameplay and browser-local scores are unchanged; the accepted Phase 3C release has no database, application session, shared ranking, or score submission API. Phase 4B provisioned the private Postgres foundation and deployed `c98e6bdd0fa4bdcb96332f3b17ec39fec679b74b`, with official scoring disabled. The core health check remains `/api/ready`; `/api/persistence/ready` separately verifies database compatibility. See [database architecture](DATABASE_ARCHITECTURE.md), [operations](DATABASE_RUNBOOK.md), and [Phase 4B acceptance evidence](PHASE_4B_DATABASE_FOUNDATION.md) for completed acceptance, final data checks, and recovery/rollback limitations.
