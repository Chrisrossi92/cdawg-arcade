...
Phase 4C is deployed at `0fd4c18` with sessions enabled, official scoring disabled and the temporary PKCE probe disabled. Live PKCE rejection/positive-control proof and desktop/pop-out lifecycle checks passed; Chrome web initial verification passed. See the evidence matrix for untested client cases. A verified session does not make a practice result official.
...
The installed Embedded App SDK's authorize contract accepts `code_challenge` and `code_challenge_method: S256`, and returns only `code`. Consequently this flow does not claim a returned state comparison. A fresh verifier is encrypted in each challenge, its SHA-256 challenge goes to SDK authorize, and the verifier goes to the server token exchange. SDK authenticate still receives only the transient access token. Current documentation lists web, iOS and Android support for authorization/authentication; desktop/pop-out behavior has owner and database evidence, and **rejection of an incorrect PKCE verifier followed by success with the original verifier** passed the live paired probe. Broader client coverage remains limited as recorded in the evidence matrix. No silent downgrade removes PKCE after a failed exchange. [Embedded SDK reference](https://docs.discord.com/developers/developer-tools/embedded-app-sdk).
...
Discord documents its proxied Activity origin and requires `SameSite=None Partitioned` for iframe cookies; stricter SameSite policies are unsuitable there. Its guidance says the cookie domain must match the full application proxy hostname. **Scope inference:** omission of Domain produces host-only scope at the browser-visible proxy hostname while retaining the `__Host-` protection. Live desktop logout/revocation and desktop/web challenge completion support compatibility in the tested clients; broader client coverage remains unverified. Partitioning and third-party-cookie restrictions vary by browser/client; local header tests do not establish live compatibility. No insecure cookie or persistent bearer fallback has been approved. [Discord networking](https://docs.discord.com/developers/activities/development-guides/networking).
...
See [deployment and live matrix](PHASE_4C_SERVER_SESSIONS.md) for completed checks and remaining client coverage and [database operations](DATABASE_RUNBOOK.md) for provider boundaries.

## Temporary PKCE enforcement probe (live proof passed; now disabled)

`ARCADE_PKCE_PROBE_ENABLED` is server-only and defaults false. During an explicitly approved test window, the existing verification path first exchanges the real SDK authorization code with a distinct, valid-format verifier. A pass requires HTTP 400 with OAuth `invalid_grant`, followed by a successful exchange of that **same code** using the original verifier and successful user/instance verification. A generic failure, timeout, invalid client, consumed code, or unsuccessful positive control is inconclusive and fails closed. No fallback removes PKCE. If the wrong verifier receives an access token, the probe attempts to revoke only that newly issued token and refuses to return a session bootstrap. Revocation failure never permits login.

The normal 7.5-second shared provider deadline, response size cap, fixed destinations, existing origins/CSRF and challenge one-use rules remain in force. The diagnostic reports only a fixed event and one of `rejected_then_verified`, `wrong_verifier_accepted`, or `inconclusive`; it never logs an authorization code, verifier, token, identity or raw provider error. A pass refers to provider verification, not necessarily later database session finalization. When disabled, it adds no request or probe log. Disable it after the controlled test. If Discord invalidates the code after the rejected exchange, this design must report inconclusive; do not call that a pass or silently weaken authentication. No new secret is needed. The controlled live probe recorded `rejected_then_verified` during the owner’s Chrome web launch. The diagnostic was then disabled and the same release redeployed; it is not part of normal production authentication traffic.
