...
export function stepBalanceSimulation(
  ...
  curve?: { difficultyMultiplier: number; boundedDisturbance: number },
): SimulationStep {
  if (previous.failed) {
  ...
  const difficultyMultiplier = curve?.difficultyMultiplier ?? calculateDifficulty(survivalMs, config);
  ...
  const disturbance = clamp(
    (curve?.boundedDisturbance ?? calculateBoundedDisturbance(survivalMs, difficultyMultiplier, config)) *
