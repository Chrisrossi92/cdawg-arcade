...
export function ArcadeApp({runtime}:{runtime:ArcadeRuntime}){
  ...
  return <div className={runtime.destination==='lobby'?'arcade-lobby':`arcade-game arcade-${runtime.phase}`} data-reduced={reduced}>
    <div className="arcade-settings">{runtime.destination==='balance'&&<button disabled={runtime.locked} onClick={()=>go('lobby')}>{runtime.phase==='playing'||runtime.phase==='countdown'?'Leave run · Back to Arcade':'Back to Arcade'}</button>}{runtime.locked&&<span role="status">Finish confirming your result before returning.</span>}</div>
    {runtime.destination==='balance'?<GameBoundary onBack={()=>go('lobby')}><Suspense fallback={<section className="arcade-load" role="status"><h1 tabIndex={-1}>Getting Balance ready…</h1><p>You can return to the Arcade while the game loads.</p></section>}><Game runtime={runtime} onBack={()=>go('lobby')}/></Suspense></GameBoundary>:<>
      ...
      <header><a className="brand-home" href="#arcade" aria-label="CDAWG ARCADE home"><img src={logo} alt="CDAWG ARCADE"/></a><nav aria-label="Main"><a href="#games">Games</a><a href="#player-summaries">Your results</a></nav><PlayerSettings context={runtime.context} official={model.official} onRetry={runtime.retryConnection} onPractice={runtime.practice} less={less} onLess={setLess}/></header>
      ...
      <section className="arcade-connection"><ConnectionStatus recoveryOnly context={runtime.context} onRetry={runtime.retryConnection} onPractice={runtime.practice}/>{runtime.connecting&&runtime.context.connectionState!=='discord-connecting'&&<p role="status">Getting your scores… <button className="quiet-button" onClick={runtime.practice}>Continue in practice</button></p>}</section>
      <section id="games" aria-labelledby="games-title"><div className="section-heading"><h2 id="games-title">Pick your good time.</h2><span className="pill">1 game · more on the way</span></div><div className="games-grid"><GameCard state={model.state} onPlay={()=>go('balance')} onRetry={runtime.context.connectionState==='discord-error'?undefined:()=>void runtime.refresh()} onAttention={()=>setAttention(n=>n+1)}/><GameCard state="soon"/><GameCard state="soon"/></div></section>
      <p className="feedback">{model.official?'Play for your place on the server leaderboard.':'Just playing? Practice scores stay on this device.'}</p>
      <section id="player-summaries" className="summaries" aria-label="Player summaries"><article><span className="eyebrow">{model.official?'YOUR OFFICIAL BEST':'YOUR PRACTICE BEST'}</span><div className="summary-body"><strong className="score">{model.best}</strong><p>{model.official?model.stats?`${model.stats.acceptedCount} official runs`:'Results unavailable':'Practice on this device'}</p></div>{model.official&&<button className="quiet-button" disabled={view.loading} onClick={()=>void runtime.refresh()}>Refresh results</button>}</article><article><span className="eyebrow">SERVER LEADERBOARD</span>{model.board?<><ol aria-label="Guild leaderboard summary">{model.board.entries.slice(0,3).map(e=><li key={e.playerTag}><span>#{e.rank} </span>{displayName(e.displayName)}{e.isYou?' · You':''}<strong> {(e.ticks/60).toFixed(3)}s</strong></li>)}</ol>{!model.board.entries.length&&<p>No official runs yet.</p>}{model.board.ownEntry&&<p>Your rank: #{model.board.ownEntry.rank}</p>}</>:<p>{model.official?view.loading?'Loading scores…':'Leaderboard temporarily unavailable.':'Open in Discord for your server’s leaderboard.'}</p>}{model.official&&<button className="quiet-button" disabled={view.loading} onClick={()=>void runtime.refresh()}>Refresh leaderboard</button>}</article></section>
